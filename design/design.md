...
**First app implementation shell.** Before the anatomical Whole Body view is ready, the simulator still ships a real app shell rather than another static placeholder. That shell has two persistent control rows — a **Sim Bar** and a **Viewer Bar** — above a main work area split into **Events** and **State**, and a persistent **bottom timeline / run bar** beneath it. The shell is not throwaway scaffolding: later views inherit the same bars, the same run model, the same persistence model, and the same history scrubber. The bottom bar follows the same rules as the top status chrome: always visible, layout-stable, and shared across views. At *Compact* the shell is a single-column stack with the Events and State panes toggled or vertically stacked; at *Standard* and above they may sit side by side.
...
**First implementation layout.** Before the anatomical body view is drawn, the same underlying model appears as a simpler **Events / State** split. The State side is organised around one **full-width Whole Body summary panel** followed by a field of body-part panels. Each body-part panel uses the same base card grammar — title, health/status chip, large instantaneous quantity graphic, and small past-values chart drawn over the time scale selected in the Viewer Bar. The default card size is standardised so panels can flow in a grid-like layout, but cards may be collapsed, shown at the standard size, or expanded for more detail.

A **left vertical selector rail** sits beside the panel field. It lists the available body parts as chips, with chip colour reflecting health or current status. The rail is global organiser rather than the only interaction path: the user can show or hide a panel, jump to it, and reopen it from the rail, and can also act directly on the cards themselves. The user's panel arrangement is part of the run state and is saved with the run.

User-controlled arrangement is allowed, but temporal instability is not: panels may be drag-reordered by the user, yet once placed they stay fixed across play, pause, scrubbing, replay, and branch comparison. The point is to let the user choose their inspection layout without sacrificing the design's position-stability rule over time.
...
**Cycle sets.** The schedule editor's top section is organised into visible cycle lanes rather than one undifferentiated repeating list. The first lane is the **daily** loop. The second is the **alternating-days** loop. The third is the **weekly** loop. Each lane shows its own day cells and the events pinned into them. This is the minimum cycle vocabulary that covers the common patterns the simulator is meant to teach: every day, every other day, and weekly routines. More exotic recurrence rules can still exist in the data model, but the first UI privileges these three because users can read them at a glance.

**Schedule above, editor below.** The Events side is vertically split. The top section is the schedule itself — day cells within the active cycle lane, with events rendered as cards or chips. Tapping *view event* opens that event into the bottom section, which is the event editor. The bottom section is also where new events are created. A cycle selector and time-of-day control determine where the event appears when saved. Adding an event immediately places it into the selected cycle lane; running the simulation activates scheduled events automatically when their simulated time arrives. Along the bottom of the page, beneath both the Events and State sides, sits the persistent timeline / run bar: the shared scrubber for recorded history, event marks, bookmarks, and run-position context.
...
**One simulation step:**
1. Apply any events firing in this step (eating, starting exercise, etc.).
...
**Adaptive step size:** At low time-multipliers the engine uses a 1-second step. At higher multipliers it switches to 1-minute, then 1-hour, then 1-day steps as the fast processes are no longer being watched. The chronic-effect logic (tissue remodeling, vessel wall changes) runs at a daily step regardless of what the fast loop is doing, so years-long runs remain accurate for the slow stuff even at extreme speeds.
