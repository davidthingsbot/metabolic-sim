...
**First implementation layout.** Before the anatomical body view is drawn, the State side is organised around one **full-width Whole Body summary panel** followed by a stack or field of body-part panels. Each body-part panel uses the same base card grammar — title, health/status chip, upper diagram area, and lower chart/history area. The default card size is standardised so panels can flow in a grid-like layout, but cards may be collapsed, shown at the standard size, or expanded for more detail.
...
**Cycle model.** The planner always supports a one-off event and a permanently repeating daily cycle. Beyond that, the repeating-cycle model is open-ended: the user can add as many extra cycle lanes as needed. The headline defaults are a **2-day alternating cycle** (useful for alternating exercise schedules), a **7-day weekly cycle**, and one or more longer custom cycles (monthly, quarterly, or any other repeating span the user wants). All repeating cycles run forever, dropping their events into the simulation at the appropriate times and for the appropriate durations.
