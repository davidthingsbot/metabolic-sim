...
**First implementation layout.** Before the anatomical body view is drawn, the same underlying model appears as a simpler **Events / State** split. The Events side is the planner and event log. The State side is the live body readout. On the State side, each critical body location gets: a title; a large instantaneous quantity graphic (bar, container fill, or equivalent); and a small past-values chart drawn over the time scale selected in the Viewer Bar. This state view is intentionally instrument-panel-like at first — enough to make the engine legible before the anatomical SVG arrives.
