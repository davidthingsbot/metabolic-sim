...
export function getBodyControlPoints(params = {}) {
  const merged = { sex: 'male', age: 35, bodyMassIndex: 24, ...params };
  const paths = buildPaths(merged);
  const transformX = 100;
  const skew = paths.stoop === 0 ? 0 : -paths.stoop * 9;
  const skewRadians = skew * Math.PI / 180;
  const applyTransform = (point, kind) => ({
    name: point.name,
    kind,
    x: transformX + point.x + Math.tan(skewRadians) * point.y,
    y: point.y,
  });
  return {
    outline: paths.contour.map((point) => applyTransform(point, 'outline')),
    detail: paths.detailControls.map((point) => applyTransform(point, 'detail')),
    proportions: paths.proportions,
  };
}
