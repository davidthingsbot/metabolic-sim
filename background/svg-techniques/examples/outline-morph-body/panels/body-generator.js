...
function proportions({ sex, age, bodyMassIndex }) {
  ...
  const crotchY = top + height * mix(0.63, 0.605, adult);
  ...
  const shoulderHalf = shoulderJointHalf + mass * mix(1.2, 2.4, adult) - lean * mix(0.3, 0.8, adult);
  const armpitHalf = shoulderHalf - mix(2.5, 6, adult);
  const waistBase = (
    mix(17, 20, adult) +
    malePuberty * 2.2 -
    femalePuberty * 2.4 +
  ...
  const bellyHalf = waistBase + mass * (mix(5, 13.5, adult) + malePuberty * 4.5 - femalePuberty * 1.2);
  const hipHalf = (
    hipJointHalf +
    mix(8, 9, adult) +
  ...
  const thighHalf = mix(9.5, 13, adult) + mass * mix(4.5, 9.5, adult) + femalePuberty * 1.4;
  const kneeHalf = mix(6.8, 8.8, adult) + mass * 2.2;
  const calfHalf = mix(7.5, 9.8, adult) + mass * 2.7;
  const ankleHalf = mix(4.5, 5.5, adult) + mass * 0.8;
  const armHalf = mix(4.8, 5.8, adult) + mass * mix(1.8, 2.7, adult);
  const handHalf = mix(3.4, 4.1, adult) + mass * 0.35;
...
function rightLandmarks(p) {
  ...
  const hipShelfX = mix(p.bellyHalf, p.hipHalf, 0.37);
  const hipOuterX = p.hipHalf * 0.86;
  const upperThighX = mix(hipOuterX, p.thighHalf + legGap * 0.95, 0.42);
